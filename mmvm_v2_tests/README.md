...
into it. A program argument is optional when `--snapshot` is present; without
one, the runner writes the image and exits. If a program is supplied, snapshot
creation occurs before that application is opened, parsed, compiled, or
...
Native source can be divided into explicitly declared helper functions. The
compiler recursively reads every dependency with `Function#toString()`, emits
the complete reachable graph into one native mapping, and lowers direct helper
calls to native-to-native calls; execution does not return to the JavaScript
interpreter between functions. For example:
...
nativeTwicePlus.nativeCompile = {
    dependencies: {nativeAdd: nativeAdd}
};

var compiled = compileNative(nativeTwicePlus);
var answer = compiled.fn(7, 3); /* 13 */
```

Dependency keys must exactly match the declared function names. A dependency
may have its own `nativeCompile.dependencies`, and the compiler traverses those
recursively. Direct self-recursion needs no declaration; mutual recursion is
expressed by listing the other function on each side. Cycles are detected while
constructing the graph rather than recursively expanding forever. Calls to an
undeclared helper and mismatched dependency names are compilation errors. This
explicit map is required because `Function#toString()` provides source text but
does not expose a function's lexical environment reliably enough to resolve
arbitrary free identifiers.

The internal ABI uses ordinary stack arguments and an `eax` return value.
Arguments are evaluated and stored left-to-right to preserve JavaScript
evaluation order. Internal helpers may take more than eight arguments; only
the root entry remains subject to the MMVM FFI bridge's eight-argument limit
and existing specialization rule. `return expression;` and `return;` are part
of the native subset. Root constants are inherited by dependencies, whose own
`nativeCompile.constants` may override them locally. Dependency specialization
is deliberately rejected because specialization belongs to the externally
invoked root variant.

Every specialized root variant owns one mapping containing its whole function
graph. Forward calls, nested calls, self-recursion, and mutual recursion use
labels and relative call fixups within that mapping. `compiled.destroy()`
therefore releases the root and every dependency together, preventing dangling
native call targets. Macro-assembly dumps include each function label and every
generated `call(...)`; variants expose `functionCount` for inspection.
...
`return`, assignment and `+=`/`-=`, postfix increment/decrement, unary `+`,
`-`, and `~`, signed comparisons in conditions, integer arithmetic, shifts,
bitwise operators, and direct calls to declared native dependencies. Direct
`peek32(address)` and `poke32(address, value)` calls lower to native memory
loads and stores. Free identifiers and member expressions may be bound to
integer constants through `function.nativeCompile.constants`.
...
`native_compiler_dependencies_test.js` is the dependency-graph regression
test. A direct Node.js run executes the same functions as ordinary JavaScript;
the `js_min.exe` invocation recursively compiles and executes all seven
functions. It covers nested dependencies, mutual recursion, native returns,
left-to-right argument evaluation, a nine-argument internal helper, graph
lifetime, native call emission, and deterministic declaration errors:

```sh
node native_compiler_dependencies_test.js

LD_LIBRARY_PATH="$MOZJS_LIB" \
  "$MMVM_ROOT/artifacts/js_min.exe" \
  demo8_runner.js native_compiler_dependencies_test.js
```
