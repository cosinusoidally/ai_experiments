/*
 * Original arcade rally stage through a procedurally generated Welsh-inspired
 * upland landscape. Rendering, physics, AI, scenery, and bitmap UI are all
 * implemented in JavaScript; X11 only presents the completed framebuffer.
 */
var common = require("./demo_common.js");

var options = common.parseOptions(process.argv, "demo6.js");
var TRACK_POINTS = 96;
...
function project(world) {
    ...
    return {x: camera.centerX + right * camera.focal * inverseZ,
            y: camera.centerY - vertical * camera.focal * inverseZ,
            inverseZ: inverseZ};
}
...
function rasterTriangle(framebuffer, first, second, third, packed) {
    ...
    if (!area) return;
    if (area < 0) {
        var swap = second; second = third; third = swap; area = -area;
    }
    ...
    var minimumY = Math.max(0, Math.floor(Math.min(first.y, second.y, third.y)));
    ...
    var edge0X = third.y - second.y;
    var edge0Y = -(third.x - second.x);
    var edge1X = first.y - third.y;
    var edge1Y = -(first.x - third.x);
    var edge2X = second.y - first.y;
    var edge2Y = -(second.x - first.x);
    var row0 = (startX - second.x) * edge0X + (startY - second.y) * edge0Y;
    var row1 = (startX - third.x) * edge1X + (startY - third.y) * edge1Y;
    var row2 = (startX - first.x) * edge2X + (startY - first.y) * edge2Y;
    ...
    for (var y = minimumY; y <= maximumY; y++) {
        ...
        for (var x = minimumX; x <= maximumX; x++) {
            if (weight0 >= 0 && weight1 >= 0 && weight2 >= 0) {
                var inverseZ = (weight0 * first.inverseZ +
                                weight1 * second.inverseZ +
                                weight2 * third.inverseZ) * inverseArea;
                var index = y * width + x;
                if (depthStamps[index] !== depthGeneration ||
                    inverseZ > depthBuffer[index]) {
                    if (pixelAddress) poke32(pixelAddress + index * 4, packed);
                    else pixels.writeUInt32LE(packed >>> 0, index * 4);
                    depthBuffer[index] = inverseZ;
                    depthStamps[index] = depthGeneration;
                }
            }
            weight0 += edge0X; weight1 += edge1X; weight2 += edge2X;
        }
        row0 += edge0Y; row1 += edge1Y; row2 += edge2Y;
    }
}

function drawQuad(framebuffer, a, b, c, d, color) {
        ...
        for (var sideIndex = 0; sideIndex < 2; sideIndex++) {
            ...
            for (var band = 0; band < bands.length - 1; band++) {
                ...
                cells.push({a: innerFirst, b: innerSecond, c: outerSecond,
                            ...
                            centerX: (innerFirst.x + innerSecond.x +
                                      outerSecond.x + outerFirst.x) * 0.25,
                            centerZ: (innerFirst.z + innerSecond.z +
...
function drawHillsides(framebuffer) {
    for (var i = 0; i < terrainCells.length; i++) {
        ...
        var minimumForward = 1e30;
        var points = [cell.a, cell.b, cell.c, cell.d];
        for (var pointIndex = 0; pointIndex < points.length; pointIndex++) {
            var dx = points[pointIndex].x - camera.x;
            var dz = points[pointIndex].z - camera.z;
            var forward = dx * camera.forwardX + dz * camera.forwardZ;
            if (forward < minimumForward) minimumForward = forward;
        }
        if (minimumForward < 1.5) continue;
...
function drawRoad(framebuffer) {
    for (var i = 0; i < TRACK_POINTS; i++) {
        var first = track[i];
        var second = track[(i + 1) % TRACK_POINTS];
        var dx = (first.x + second.x) * 0.5 - camera.x;
        var dz = (first.z + second.z) * 0.5 - camera.z;
        if (dx * dx + dz * dz > 90 * 90) continue;
        var shoulderColor = (i & 1) ? 0x72583a : 0x674d33;
        var roadColor = (i % 3) ? 0x8b7455 : 0x806949;
        drawQuad(framebuffer, roadEdge(first, -4.45, -0.05),
                 roadEdge(second, -4.45, -0.05),
                 roadEdge(second, 4.45, -0.05), roadEdge(first, 4.45, -0.05),
                 shoulderColor);
        drawQuad(framebuffer, roadEdge(first, -ROAD_HALF_WIDTH, 0),
                 roadEdge(second, -ROAD_HALF_WIDTH, 0),
                 roadEdge(second, ROAD_HALF_WIDTH, 0),
                 roadEdge(first, ROAD_HALF_WIDTH, 0), roadColor);
        var rutColor = (i & 3) ? 0x5d4b37 : 0x68523a;
        drawQuad(framebuffer, roadEdge(first, -1.38, 0.018),
                 roadEdge(second, -1.38, 0.018), roadEdge(second, -0.82, 0.018),
                 roadEdge(first, -0.82, 0.018), rutColor);
        drawQuad(framebuffer, roadEdge(first, 0.82, 0.018),
                 roadEdge(second, 0.82, 0.018), roadEdge(second, 1.38, 0.018),
                 roadEdge(first, 1.38, 0.018), rutColor);
        if ((i % 5) === 2) {
            drawQuad(framebuffer, roadEdge(first, -0.38, 0.022),
        ...
        drawWallSection(framebuffer, first, second, -WALL_OFFSET, wallShade);
        drawWallSection(framebuffer, first, second, WALL_OFFSET, wallShade);
    }
}

function drawWallSection(framebuffer, first, second, offset, color) {
    var bottomFirst = roadEdge(first, offset, 0);
    var bottomSecond = roadEdge(second, offset, 0);
    var middleFirst = {x: bottomFirst.x, y: bottomFirst.y + 0.43, z: bottomFirst.z};
    var middleSecond = {x: bottomSecond.x, y: bottomSecond.y + 0.43, z: bottomSecond.z};
    var topFirst = {x: bottomFirst.x, y: bottomFirst.y + 0.76, z: bottomFirst.z};
    var topSecond = {x: bottomSecond.x, y: bottomSecond.y + 0.76, z: bottomSecond.z};
    drawQuad(framebuffer, bottomFirst, bottomSecond,
             middleSecond, middleFirst, shadeColor(color, 0.73));
    drawQuad(framebuffer, middleFirst, middleSecond, topSecond, topFirst, color);
}
