...
Build a JavaScript VM written in the mawkcc bootstrap dialect, then use it
as another bootstrap route for `../mawkcc/mawkcc_self.c`.

The first VM is intentionally not an ECMAScript implementation. It only
needs to load and execute the JavaScript-shaped subset already used by
`../mawkcc/mawkcc_self.c` plus a small host runtime equivalent to
`../mawkcc/scripts/mawkcc-js-runner.js`. After that route is stable, extend the
language in small audited steps until the VM reaches ECMAScript 5.1.
...
restricted dialect as `../mawkcc/mawkcc_self.c`.
...
That route should compile or run `jsvm_self.c`, ask it to execute
`../mawkcc/mawkcc_self.c`, and write the generated mawkcc executable. Once stable,
...
For the initial VM, compile source into a compact internal bytecode instead
of walking an AST directly. A bytecode interpreter is easier to checkpoint,
trace, compare, and later retarget than a recursive tree evaluator.
...
Before writing the VM, mechanically inventory what `../mawkcc/mawkcc_self.c` actually
uses as JavaScript:
...
## Phase 1: Host-Built Prototype

Build the first VM logic in a convenient host language or in plain C if that
accelerates validation. This prototype is disposable; its job is to settle
semantics and test data before the mawkcc-dialect port.

Minimum prototype behavior:

1. parse the bootstrap subset
2. compile to internal bytecode
3. execute bytecode with signed 32-bit integer arithmetic
4. provide the same primitive layer as `../mawkcc/scripts/mawkcc-js-runner.js`
5. run `../mawkcc/mawkcc_self.c` and emit a mawkcc binary byte-identical to
   `../mawkcc/scripts/build-mawkcc-js.sh`

Keep prototype tests small and language-oriented:

- arithmetic and comparison primitives
- recursion and nested calls
- global assignment
- while loops and break
- string literal interning through `mks`
- byte memory through `brk`, `ri8`, `wi8`, `ri32`, and `wi32`
- file read and stdout write behavior used by the compiler

Exit criteria:

- the prototype runs `../mawkcc/mawkcc_self.c`
- output matches the SpiderMonkey JS route byte-for-byte
- every prototype-only semantic decision is written down before porting


## Phase 2: Port The VM To The Bootstrap Dialect

Create `jsvm_self.c` in the restricted source shape documented by
...
9. primitive runtime
10. command-line `main(argc, argv)`

At each layer, add a tiny input fixture and a script that can run the VM
under at least one existing route. Early scripts can use host support or a
temporary trace mode; the important thing is to avoid waiting until the full
compiler run to find parser bugs.

Exit criteria:

- `jsvm_self.c` compiles with `../mawkcc/cc.awk`
- the compiled VM can execute small bootstrap-subset programs
- no host-only data structures or hidden JavaScript behavior remain
...
2. execute the resulting VM with arguments equivalent to the SpiderMonkey
   runner: input `../mawkcc/mawkcc_self.c`, output path for the emitted compiler
...
- close and exit
- synthetic `argv` containing the VM route name and source path
...
This is not full JavaScript self-hosting yet, but it proves the VM source is
inside the bootstrap closure.
...
- no dependency from `../mawkcc/mawkcc_self.c` until the feature is verified
...
1. Generate and check in a `../mawkcc/mawkcc_self.c` JavaScript-subset inventory.
2. Decide final filenames for the VM source, tests, and build script.
3. Prototype the subset parser and bytecode format.
4. Prove the prototype can emit a byte-identical mawkcc compiler.
5. Port the lexer and bytecode interpreter into `jsvm_self.c`.
6. Add small VM fixtures before attempting the full compiler run.
7. Add `../mawkcc/scripts/build-mawkcc-jsvm.sh`.
8. Add the JS VM route to verification only after it is byte-identical.
