...
int main()
{
    try {
        ...
        require(std::vector<std::uint8_t>{writer.begin(), writer.end()} == expected,
                "little-endian append failed");

        writer.patch_u32_le(3, 0x01020304);
        const std::vector<std::uint8_t> patched{
            0x12, 0x56, 0x34, 0x04, 0x03, 0x02, 0x01};
        require(std::vector<std::uint8_t>{writer.begin(), writer.end()} == patched,
                "little-endian patch failed");

        writer.pad_to(10);
        require(writer.size() == 10, "padding produced the wrong size");
        ...
        require(rejected_bad_patch, "out-of-bounds patch was accepted");
        ...
        require(rejected_backwards_padding, "backwards padding was accepted");
