...
log_cmd "mawk -f \"$ROOT/cc.awk\" -- -c \"$ROOT/mawkcc_self.c\" -o \"$SELF_OBJ\""
mawk -f "$ROOT/cc.awk" -- -c "$ROOT/mawkcc_self.c" -o "$SELF_OBJ"

log_cmd "\"$CC_BIN\" -c \"$ROOT/mawkcc_self.c\" -o \"$SELF_ORIG_OBJ\""
"$CC_BIN" -c "$ROOT/mawkcc_self.c" -o "$SELF_ORIG_OBJ"
...
log_cmd "mawk -f \"$ROOT/cc.awk\" -- \"$ROOT/mawkcc_self.c\" -o \"$SELF_BIN\""
mawk -f "$ROOT/cc.awk" -- "$ROOT/mawkcc_self.c" -o "$SELF_BIN"
chmod +x "$SELF_BIN"

log_cmd "\"$SELF_BIN\" -c \"$ROOT/mawkcc_self.c\" -o \"$SELF_REBUILT_OBJ\""
"$SELF_BIN" -c "$ROOT/mawkcc_self.c" -o "$SELF_REBUILT_OBJ"
...
log_cmd "\"$SELF_GCC_BIN\" -c \"$ROOT/mawkcc_self.c\" -o \"$SELF_GCC_REBUILT_OBJ\""
"$SELF_GCC_BIN" -c "$ROOT/mawkcc_self.c" -o "$SELF_GCC_REBUILT_OBJ"
...
for src in "$ROOT"/examples/*.c; do
    ...
    mawk -f "$ROOT/cc.awk" -- "$src" -o "$AWK_BIN"
    "$CC_BIN" "$src" -o "$C_BIN"
...
mawk -f "$ROOT/cc.awk" -- -c "$OBJ_SRC" -o "$AWK_OBJ_OUT"
"$CC_BIN" -c "$OBJ_SRC" -o "$C_OBJ_OUT"
"$SELF_BIN" -c "$OBJ_SRC" -o "$SELF_OBJ_OUT"
